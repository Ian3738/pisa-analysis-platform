...
build_report <- function() {
  ...
  ai_med <- function(d, i) median(d[idx == i]$pct, na.rm = TRUE)
  # 臺灣在某一份問卷上的名次表（依名次排序）
  ...
  doc <- add_body(doc, sprintf(
    "過去 12 個月使用過人工智慧的國中教師比率，國際中位數為 %.1f%%，最高為阿拉伯聯合大公國 %.1f%%，最低為法國 %.1f%%。東亞四個參與者之間的落差極大：新加坡 %.1f%%、越南 %.1f%%、韓國 %.1f%%、日本 %.1f%%。日本在 %d 個參與者中排名倒數第 2，與新加坡相差 %.1f 個百分點。此一差距顯示以「東亞」作為分析單位並不恰當，區域內的異質性大於區域間。",
    median(AIU$pct), max(AIU$pct), min(AIU$pct), sg$pct, AIU[CNT=="VNM"]$pct,
    AIU[CNT=="KOR"]$pct, jp$pct, nrow(AIU), sg$pct - jp$pct))
  doc <- add_figure(doc, fn, "ai_usage",
    "過去 12 個月使用過人工智慧的國中教師比率",
    "橫線為 95% 信賴區間。分母為人工智慧模組的全體作答教師。臺灣未參加 TALIS 2024。"); fn <- fn + 1
  doc <- add_body(doc, sprintf(
  ...
  doc <- add_figure(doc, fn, "ai_use_what",
    "教師運用人工智慧的用途",
    "點為 55 個參與者的中位數，灰帶為全距。分母為全體作答教師，未使用人工智慧者一律計為 0，故各項比率可直接相互比較。"); fn <- fn + 1
  ...
  doc <- add_body(doc, sprintf(
    ...
    median(AIK$pct), max(AIK$pct)))
  doc <- add_figure(doc, fn, "ai_belief",
    "教師對人工智慧在教育中的信念：十個題目的國際同意率",
    "同意率的分母已排除回答「不知道」者。點為 55 個參與者的中位數，灰帶為全距。"); fn <- fn + 1
  doc <- add_body(doc,
  ...
  doc <- add_figure(doc, fn, "ai_use_belief",
    "正向信念與使用率的國家層級關聯",
    "每一點為一個參與者。橫軸為五項正向信念的平均同意率，縱軸為使用率。東亞四個參與者以強調色標示。"); fn <- fn + 1
  ...
  doc <- add_figure(doc, fn, "ai_why_not",
    "未使用人工智慧的教師所述的原因",
    "分母為未使用人工智慧的教師，非全體教師。題目可複選，故各項比率合計超過 100%。"); fn <- fn + 1
  doc <- add_body(doc, sprintf(
  ...
  doc <- add_body(doc, sprintf(
    "第八，教師使用人工智慧的資料目前僅有一份，而臺灣不在其中。TALIS 2024 的教師問卷是本研究涵蓋的五套評比中唯一含有人工智慧題目者，共 %d 題、%d 個參與者。國際中位數顯示 %.1f%% 的國中教師在過去 12 個月使用過人工智慧，用途集中於備課（摘要主題 %.1f%%、產生教案 %.1f%%）而非評量（批改作業 %.1f%%）。未使用者中 %.1f%% 自述缺乏知識與技能，而 %d 個參與者中有 %d 個的專業發展需求高於供給。臺灣未參加該輪，因此無法據此判斷臺灣教師的狀況。",
    AID$meta$n_item, AID$meta$n_country, median(AIU$pct),
    ai_med(AIW, "TT4G37B"), ai_med(AIW, "TT4G37C"), ai_med(AIW, "TT4G37A"),
