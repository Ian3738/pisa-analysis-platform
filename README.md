...
| 項目 | 內容 |
|---|---|
| 資料庫 | PISA 2015／2018／2022、TIMSS 2023 四與八年級、PIRLS 2021、ICCS 2016／2022、TALIS 2018／2024 |
| 選考領域 | PISA 2022 創造思考（63 國）、財金素養（20 國） |
| 樣本 | PISA 174.5 萬、TIMSS 72.0 萬、PIRLS 36.8 萬、ICCS 17.7 萬名學生，合計逾 300 萬；另 TALIS 19.5 萬、TIMSS／PIRLS／ICCS 10.1 萬名教師與校長 |
| 儲存 | 632 MB Parquet（原始 `.sav` 合計約 6 GB） |
| 倉儲 | DuckDB，view 直接查 Parquet |
| 問卷 | 學生、家長、教師、校長四類問卷；TIMSS 15 個、PIRLS 19 個、ICCS 30 個合成量表 |
...
| 圖表 | 150 張 SVG（亮暗各一版）+ 75 張 200 dpi PNG，全部由 ggplot2 產出 |
| 報告 | APA 第七版 Word 檔，75 圖、19 表，每項均附研究結果說明 |
...
│   ├── 13_figures_ai.R       AI 專區的圖表
│   ├── 14_figure_rankorder.R 各單位內部的用途排序比較（Kendall W）
...
│       ├── 06_analysis_ai.R  TALIS 2024 的人工智慧題組分析
...
├── web/
│   ├── template.html         網站樣板
│   ├── figs/                 148 張 SVG（亮暗各一版）
│   ├── png/                  74 張 200 dpi PNG（Word 報告用）
...
把五套評比的變數標籤逐檔搜過（比對 `artificial intelligence`、`AI`、`chatbot`、
`machine learning`、`generative`、`ChatGPT`、`large language`），答案只有一個：

| 資料庫 | 問卷 | 變數數 | AI 題 | ICT 題（對照） |
|---|---|---|---|---|
| **TALIS 2024** | **教師** | 611 | **28** | — |
| TALIS 2024 | 校長 | 473 | 0 | — |
| TALIS 2018 | 教師 | 493 | 0 | 11 |
| PISA 2022 | 學生 | 1,278 | 0 | 136 |
| PISA 2022 | 學校 | 431 | 0 | 37 |
| TIMSS 2023 · PIRLS 2021 · ICCS 2016／2022 | 學生／家長／教師／校長 | 2,589 | 0 | — |

ICT 欄是對照組：PISA 2022 學生問卷有 136 題 ICT 卻 0 題 AI，證明搜尋有效，
不是比對字串寫錯。

**臺灣未參加 TALIS 2024**，所以目前唯一一份跨國、具代表性抽樣的教師 AI 使用
資料裡沒有臺灣。

幾個結果：使用率跨單位中位數 **42.2%**，範圍從法國 13.5% 到阿聯 75.0%；
東亞內部差距比區域間更大（新加坡 74.9%、越南 64.5%、上海 53.5%、韓國 42.7%、
**日本 17.4%——倒數第 2**，五者相差 57.5 個百分點）。用途集中在備課（摘要主題 26.1%、產生教案 25.4%），
最少用於評閱作業（9.9%）。而且這個**先後順序在各單位內部高度一致**：
Kendall 和諧係數 W = 0.651，備課兩項的最佳名次在 53 個單位中
**全數優於評量兩項，例外為零**。同意率最高的兩題都是風險（助長抄襲 85.1%、
錯誤建議 85.0%），而正向信念與風險認知幾乎無關（**r = −0.19**）——看見助益與
看見風險是兩件事，不是同一條軸的兩端。未使用者中 **69.3% 說自己缺技能**，
遠高於缺設備（39.2%）與學校禁止（12.1%）；而 **53 個單位有 51 個的
專業發展需求高於供給**。

兩個容易誤判的設計。其一，AI 模組的有效作答只有 32.3%，看起來像嚴重無回應，
實際是**輪換題本**——55 國全部施測，每位教師只拿到三份題本之一。答題者與
未答題者的平均權重為 28.18 與 28.11，確認隨機分派，沿用 `TCHWGT` 即為不偏估計。
其二，**參與單位不全是主權國家**：比利時同時以整體與兩個語言社群出現，三者樣本
重疊（整體 1,663 人＝荷語區 798＋法語區 865），跨單位的中位數與計數因此一律以
**53 個不重複計算的單位**為基礎，逐單位的數值則保留全部 55 個以利與官方報告對照。

## 問卷告訴我們的另一半

成就分數只是資料的一半。把學生、家長、教師與校長的問卷一起看，臺灣的圖像變得複雜許多。

**成就與學習情意反向。** TIMSS 2023 八年級，臺灣數學成就排第 2，但數學自信排
**第 46**（共 47）、重視數學第 45、喜歡學數學第 40。四年級的形態相同而幅度較小，
PIRLS 的閱讀自信（第 57／64）也一致。這不是單一科目或單一年齡的現象。

**家長陪讀少，孩子識字不差。** PIRLS 2021，臺灣家長回報的入學前讀寫活動排
**第 59**（共 61）、入學前數學活動第 57、家長喜歡閱讀第 58；但「入學前識字任務」
——孩子實際能完成的事——排第 24，高於中位數。臺灣家長問卷回收率 99.4%
（全體中位數 89.6%），不是低回收造成的偏誤。

**校長與教師看到的不是同一所學校。** PIRLS 中，校長評的學校紀律與教師評的
學校安全有序，跨國相關只有 **r = 0.21**。臺灣是落差最大者之一：校長排第 5，
教師排第 54，教師工作滿意度排第 58（共 61）。用單一來源推論學校氣氛會得到
不同的答案。

**排名第 1 有兩種可能。** ICCS 2022，臺灣在 30 個公民態度量表中 11 個排第 1、
6 個排第 2，整體平均 52.91 是 24 個參與者最高。把各國的剖面各自置中（扣掉整體
作答傾向）後：與學校生活直接相關的四項**仍是第 1**，但預期參與合法抗議、校外討論
政治議題、預期積極政治參與、以數位媒體參與公共議題**全部落到第 24**。兩種算法
都不是唯一的真相，並列才能框出結論的範圍。

**第 1 名有一部分是別人掉下來。** ICCS 2016→2022 的 15 個共同參與者中，
**7 個顯著下滑、沒有任何一個上升**（挪威 −34.5、丹麥 −30.4、哥倫比亞 −30.0）。
臺灣 +1.9 分未達顯著，屬持平。2016 年丹麥 586.3 分還在臺灣 581.1 分之前。
