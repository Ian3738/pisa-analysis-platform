...
| 圖表 | 148 張 SVG（亮暗各一版）+ 74 張 200 dpi PNG，全部由 ggplot2 產出 |
| 報告 | APA 第七版 Word 檔，74 圖、19 表，每項均附研究結果說明 |
...
幾個結果：使用率國際中位數 **39.7%**，範圍從法國 13.5% 到阿聯 75.0%；
東亞內部差距比區域間更大（新加坡 74.9%、越南 64.5%、韓國 42.7%、
**日本 17.4%——倒數第 2**）。用途集中在備課（摘要主題 25.9%、產生教案 25.3%），
最少用於評閱作業（9.8%）。同意率最高的兩題都是風險（助長抄襲 85.1%、
錯誤建議 85.0%），而正向信念與風險認知幾乎無關（**r = −0.19**）——看見助益與
看見風險是兩件事，不是同一條軸的兩端。未使用者中 **69.5% 說自己缺技能**，
遠高於缺設備（39.1%）與學校禁止（11.6%）；而 **55 個參與者有 53 個的
專業發展需求高於供給**。

一個容易誤判的設計：AI 模組的有效作答只有 32.3%，看起來像嚴重無回應，
