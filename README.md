...
PISA、TIMSS 與 PIRLS 的整合分析平台。從官方檔案伺服器下載、轉檔、建立倉儲，
...
| 項目 | 內容 |
|---|---|
| 資料庫 | PISA 2015／2018／2022、TIMSS 2023 四與八年級、PIRLS 2021 |
| 選考領域 | PISA 2022 創造思考（63 國）、財金素養（20 國） |
| 樣本 | PISA 174.5 萬、TIMSS 72.0 萬、PIRLS 36.8 萬，合計逾 283 萬名學生 |
| 儲存 | 632 MB Parquet（原始 `.sav` 合計約 6 GB） |
| 倉儲 | DuckDB，view 直接查 Parquet |
| 估計核心 | `lib_pisa.R`（10 個推估值 × Fay BRR）、`lib_timss.R`（5 個推估值 × JK2 折刀） |
| 驗證 | PISA 與 `intsvy`、`survey_mean` 三方一致；TIMSS 與獨立實作逐位數一致 |
| 圖表 | 96 張 SVG（亮暗各一版）+ 48 張 200 dpi PNG，全部由 ggplot2 產出 |
| 報告 | APA 第七版 Word 檔，48 圖、10 表，每項均附研究結果說明 |
